...
namespace SQLiter
{
	...
	public class SQLite : MonoBehaviour
	{
		...
		/// </summary>
		private const string SQL_DB_NAME = "game";

		// table name
		private const string SQL_TABLE_NAME = "gamesQues";

		/// <summary>
		/// predefine columns here to there are no typos
		/// </summary>
		private const string COL_QUES = "question";  // using name as example of primary, unique, key
		private const string COL_OP1 = "op1";
		private const string COL_OP2 = "op2";
		private const string COL_OP3 = "op3";
		private const string COL_OP_COR = "correct";
		private const string COL_CHK = "check";
		...
		void Test()
		{
			...
			//InsertPlayer("Alex", 3, 2, 3, 2, 4);
			//InsertPlayer("Bob", 3, 2, 3, 2, 4);
			//InsertPlayer("Frank", 3, 2, 3, 2, 4);
			//InsertPlayer("Joe", 3, 2, 3, 2, 4);
		}
		...
		private void SQLiteInit()
		{
			...
			_command.CommandText = "SELECT * FROM gamesQues";
			_reader = _command.ExecuteReader();
			if (!_reader.Read())
			{
		...
		/// <param name="xp"></param>
        public void InsertPlayer(string name, string raceType, string classType, string login, string level, string xp)
		{
			name = name.ToLower();
		...
		public string QueryString(string column, string value)
		{
			...
			_connection.Open();
			_command.CommandText = "SELECT " + column + " FROM " + SQL_TABLE_NAME + " WHERE " + COL_QUES + "='" + value + "'";
			_reader = _command.ExecuteReader();
			if (_reader.Read())
		...
		{
			ExecuteNonQuery("UPDATE OR REPLACE " + SQL_TABLE_NAME + " SET " + column + "=" + value + " WHERE " + COL_QUES + "='" + name + "'");
		}
		...
		{
			ExecuteNonQuery("DELETE FROM " + SQL_TABLE_NAME + " WHERE " + COL_QUES + "='" + nameKey + "'");
		}
		#endregion
